...
$text = $blowfish->decrypt($cipher); //$text == 'This is plain text'
echo $cipher . '<br>';
echo $text;
*/
$blowfish = new blowfish();
$key = $_GET['key'];
$keyhash = sha1($key);
$blowfish->keys($keyhash);
$text = $_GET['text'];
//echo $keyhash;
echo $keyhash . "  " . $blowfish->decrypt($text);
...
class Blowfish
{
    ...
    function decrypt($text)
    {
		...
		{
			$return = $this->block_decrypt(array($cipher[$i],$cipher[$i+1]));

			if($this->encryptmode == 1)
			{
				//Xor Verkn�pfung von $return und Geheimtext aus von den letzten beiden Bl�cken
    ...
    function sbox_round($integer)
    {
        ...
        //$return = ($this->sbox0[$b0] + $this->sbox1[$b1] % pow(2,32));
        //$return = ($return ^ $this->sbox2[$b2]) + $this->sbox3[$b3] % pow(2,32);
